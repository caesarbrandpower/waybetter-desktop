...
internal struct WelcomeView: View {
    ...
    private var headerSection: some View {
        VStack(spacing: 8) {
            ...
            Text("Welcome to Waybetter Desktop")
                .font(.largeTitle)
                .fontWeight(.semibold)
            ...
            Label {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Privacy-First Local Transcription")
                        .font(.headline)
                    Text("Waybetter Desktop uses Apple's Neural Engine to transcribe audio locally on your Mac. Your audio never leaves your device.")
                        .font(.callout)
                        .foregroundStyle(.secondary)
                }
            } icon: {
    ...
    private var setupOptions: some View {
        VStack(alignment: .leading, spacing: 12) {
            Label {
                Text("Waybetter Desktop will use local AI transcription by default. No API keys or internet connection required!")
                    .font(.callout)
            } icon: {
    ...
    private var smartPasteInstructions: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Waybetter Desktop can automatically paste transcribed text using CGEvent-based automation:")
                .font(.callout)
